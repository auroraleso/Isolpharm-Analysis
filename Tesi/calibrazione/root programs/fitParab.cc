...
void par() {
  ...
  TGraphErrors *g = new TGraphErrors("calibEuLa.txt");
...
TMultiGraph* mg = new TMultiGraph();
    mg->Add(g);
    mg->Draw("ap");
    mg->SetTitle("Calibrazione detector LaBr3");
  ...
  TF1 *zero = new TF1("zero", fitzero3, 0, 2900, 2);
    zero->SetLineStyle(9);
    zero->SetLineColor(7);
   TMultiGraph* mg1 = new TMultiGraph();
    mg1->Add(gr);
    mg1->Draw("AP");
    mg1->SetTitle("Residui calibrazione detector LaBr3 ");
    mg1->GetXaxis()->SetTitle("Mean value (ADC count)");
