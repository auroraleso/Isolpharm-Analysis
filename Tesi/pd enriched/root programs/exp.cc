...
{
   double_t fitval2 = par[0] + par[1]* exp(par[2]*x[0]);
  return fitval2;
}
...
void exp()) {
  ...
  TGraphErrors *g = new TGraphErrors("all342.txt");
  ...
  TF1 *f1 = new TF1("f1", fitexp, 323288, 482308, 3);
  f1->SetLineStyle(1);
    f1->SetLineColor(2);
    f1->SetLineWidth(2);
...
TMultiGraph* mg = new TMultiGraph();
    mg->Add(g);
    mg->Draw("ap");
    mg->SetTitle("Pd-enriched,1-6 data samples,  Area 342 peak versus time for HPGe");
    mg->GetYaxis()->SetTitle("Activity [Bq]");
    mg->GetXaxis()->SetTitle("T [s] from irradiation");
    f1->Draw("same");
  ...
  TF1 *zero = new TF1("zero", fitzero2, 323288, 482308, 2);
    zero->SetLineStyle(9);
    zero->SetLineColor(7);
   TMultiGraph* mg1 = new TMultiGraph();
    mg1->Add(gr);
    mg1->Draw("AP");
    mg1->SetTitle("Pd-enriched, 1-6 data samples,  Residuals from area 342 peak versus time for HPGe");
    mg1->GetXaxis()->SetTitle("T [s] from irradiation");
 ...
 // c2->Print("Residuiobliq.pdf");
  ...
  // la chiamo f1 e la associo alla funzione esponenziale che ho creato prima di nome fitf2. 
  //Devo dare i parametri xmin e xmax
  TF1 *f4 = new TF1("f4", fitexp,  323288, 482308, 2);
  f4->SetLineStyle(1);
    f4->SetLineColor(2);
    f4->SetLineWidth(1);
  // setta i valori iniziali dei parametri
...
TMultiGraph* mg4 = new TMultiGraph();
  ...
  TF1 *zero1 = new TF1("zero1", fitzero2,  323288, 482308, 2);
