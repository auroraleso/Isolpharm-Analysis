...
void lin() {
  ...
  TGraphErrors *g = new TGraphErrors("172.txt");
 ...
 // prendo l'errore del fit che mi hp0dato sul parametro
  ...
  std::cout << "\nParametri retta "
            << p0<< " +- " << ep0
            << p1<< " +- " << ep1<<std::endl
            <<" tempo dimezzamento= "<<-log(2)/p1/3600
            << std::endl;
...
TMultiGraph* mg = new TMultiGraph();
    mg->Add(g);
    mg->Draw("ap");
    mg->SetTitle("Pd-enriched,  Area 172 peak versus time for HPGe");
  ...
  TF1 *zero = new TF1("zero", fitzero, min, max, 2);
   ...
   TMultiGraph* mg1 = new TMultiGraph();
    mg1->Add(gr);
    mg1->Draw("AP");
    mg1->SetTitle("Pd-enriched,  Residuals from area 172 peak versus time for HPGe");
 ...
 // c2->Print("Residuiobliq.pdf");
    ...
    /* TGraphErrors *g4 = new TGraphErrors("all172.txt");
    // associo un nome al seti di dati, do' un nome al puntatore del TGraphErrors
  g4->SetName("fit2");
  ...
  // la chiamo f1 e la associo alla funzione esponenziale che ho creato prima di nome fitf2. 
  //Devo dare i parametri xmin e xmax
  TF1 *f4 = new TF1("f4", fitfretta,  3232172, 482308, 2);
  f4->SetLineStyle(1);
    f4->SetLineColor(2);
    f4->SetLineWidth(1);
  // setta i valori iniziali dei parametri
...
TMultiGraph* mg4 = new TMultiGraph();
    mg4->Add(g4);
    mg4->Draw("ap");
    mg4->SetTitle("Pd-enriched, 1-6 data samples,  Area 172 peak versus time for HPGe");
  ...
  TF1 *zero1 = new TF1("zero1", fitzero,  3232172, 482308, 2);
    zero1->SetLineStyle(9);
    zero1->SetLineColor(7);
   TMultiGraph* mg14 = new TMultiGraph();
    mg14->Add(gr5);
    mg14->Draw("AP");
    mg14->SetTitle("Pd-enriched, 1-6 data samples,  Residuals from area 172 peak versus time for HPGe");
