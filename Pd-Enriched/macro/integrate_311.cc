void integrate(std::string filename, double energy1, double energy2, int step, bool ag)
{
    ...
    while( (key = (TKey*) next() ) )
    {
      ...
      TF1 * fDGausExp = new TF1("fDGausExp","[0]*exp(-0.5*( (x-[1])*(x-[1]) )/([2]*[2]) ) + [3]*exp(-0.5*( (x-[4])*(x-[4]) )/([5]*[5]) ) + [6]*exp(-[7]*x)", energy1-30, energy2+40);
      h->Fit("fExp","","",260, 280);
      h->Fit("fExp","","",260, 280);
      ...
      h->GetXaxis()->SetRangeUser(energy1-30, energy2+40);
      ...
      double counts1 = (fGauss_1->Integral(fGauss_1->GetParameter(1)-1*abs(fGauss_1->GetParameter(2)), fGauss_1->GetParameter(1)+1*abs(fGauss_1->GetParameter(2))) ) / 1.410091;
      double counts2 = (fGauss_2->Integral(fGauss_2->GetParameter(1)-1*abs(fGauss_2->GetParameter(2)), fGauss_2->GetParameter(1)+1*abs(fGauss_2->GetParameter(2))) ) / 1.410091;
    ...
    g1->Draw();
    cout << "chi square: " << fLinear_1->GetChisquare()/fLinear_1->GetNDF() << endl;
